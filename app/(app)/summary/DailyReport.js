...
export default function DailyReport({
  ...
  const totalHqCash =
    warehouseBalance?.warehouse?.find((w) => w.id === 1)?.cash || 0;
  ...
  const filteredCorpCashFlows = (corpCashFlows || []).filter(
    (cashFlow) => cashFlow.is_corporate === 1,
  );
  ...
  const totalCorpIncome = corpIncomes.reduce(
    (total, item) => total + Number(item.amount || 0),
    0,
  );

  const totalCorpExpense = corpExpenses.reduce(
    (total, item) => total + Number(item.amount || 0),
    0,
  );
  ...
  const filteredFinances = finances?.filter(
    (item) =>
      new Date(item.date_issued).toDateString() ===
        new Date(date).toDateString() &&
      ["EmployeeReceivable", "InstallmentReceivable"].includes(
        item.finance_type,
      ),
  );

  const totalReceivable = (filteredFinances || []).reduce(
    (sum, finance) => sum + Number(finance.bill_amount || 0),
    0,
  );

  const totalFee =
    revenue?.revenue?.reduce((sum, item) => sum + Number(item.fee || 0), 0) || 0;
  ...
  const netProfit =
    Number(totalFee) +
    Number(totalCorpIncome) -
    Number(totalReceivable) -
    Number(totalCorpExpense + diffTotal);
  ...
  return (
    <div className="space-y-4 font-sans text-slate-800">
      ...
      <div
        ref={reportRef}
        className="p-5 bg-white rounded-2xl border border-indigo-100 space-y-4 font-sans text-slate-900"
      >
        ...
        <div className="grid grid-cols-4 gap-2.5">
          ...
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
            <div>
              ...
              <span className="text-sm font-bold font-mono text-indigo-950 mt-0.5 block">
                {revenue?.revenue?.length || 0} Cabang
              </span>
            </div>
          ...
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
            <div>
              ...
              <span className="text-sm font-bold font-mono text-rose-600 mt-0.5 block">
                Rp {formatNumber(sumByTrxType("expense"))}
              </span>
            </div>
          ...
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
            <div>
              ...
              <span className="text-sm font-bold font-mono text-amber-600 mt-0.5 block">
                Rp {formatNumber(totalReceivable)}
              </span>
            </div>
          ...
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
            <div>
              ...
              <span className="text-sm font-bold font-mono text-indigo-600 mt-0.5 block">
                Rp {formatNumber(totalCorpExpense + diffTotal)}
              </span>
            </div>
        ...
        <div className="p-4 rounded-xl bg-indigo-700 text-white flex items-center justify-between gap-4">
          ...
          <div className="text-right text-[10px] font-mono text-indigo-200/70 space-y-0.5 border-l border-indigo-300/60 pl-4">
            ...
            <p className="text-indigo-300/80">
              Total Laba (+ Income) - (Kasbon + Corp Expense)
            </p>
          </div>
        </div>
            ...
            <table className="w-full text-left text-xs font-mono">
                ...
                {hasData ? (
                  revenue.revenue.map((item, i) => (
                    <tr key={i} className="hover:bg-indigo-50/30">
                      <td className="px-3 py-2 font-sans font-medium text-slate-700">
                        {item.warehouse.replace(/^konter\s*/i, "")}
                      </td>
                      <td className="px-3 py-2 text-right text-slate-400">
                        {formatNumber(item.expense)}
                      </td>
                      <td className="px-3 py-2 text-right font-bold text-indigo-950">
                        Rp {formatNumber(item.fee)}
                      </td>
                    </tr>
                  ))
                ) : (
              ...
              {hasData && (
                <tfoot className="bg-indigo-50/50 border-t border-indigo-100 font-bold">
                  <tr>
                    <td className="px-3 py-2 font-sans text-indigo-950">
                      TOTAL
                    </td>
                    ...
                    <td className="px-3 py-2 text-right text-indigo-600">
                      Rp {formatNumber(sumByTrxType("fee"))}
                    </td>
                  </tr>
                </tfoot>
              )}
          ...
          <div className="col-span-5 space-y-3">
                  ...
                  filteredFinances.map((finance) => (
                    ...
                    >
                      ...
                      <span className="font-bold text-amber-600">
                        Rp {formatNumber(finance.bill_amount)}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="p-2.5 text-center text-[11px] font-sans text-slate-400">
                    Tidak ada kasbon.
                  </p>
                )}
              </div>
                  ...
                  corpIncomes.map((cashflow) => (
                    ...
                    >
                      ...
                      <span className="font-bold text-emerald-600">
                        Rp {formatNumber(cashflow.amount)}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="p-2.5 text-center text-[11px] font-sans text-slate-400">
                    Tidak ada pemasukan corporate.
                  </p>
                )}
              </div>

              <div className="px-3.5 py-2 bg-indigo-50/50 border-t border-indigo-100 flex justify-between font-mono font-bold text-xs">
                <span className="font-sans text-indigo-950">
                  Total Corporate Income
                </span>
                <span className="text-emerald-600">
                  Rp {formatNumber(totalCorpIncome)}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-indigo-100 overflow-hidden bg-white shadow-xs">
              <div className="px-3.5 py-2 border-b border-indigo-100 bg-indigo-50/40 flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-slate-400">
                  {corpExpenses.length + (diffTotal !== 0 ? 1 : 0)} Item
                </span>
              </div>
                ...
                {cashdetail?.length > 0 || diffTotal !== 0 ? (
                  <>
                    <div className="px-3.5 py-2 flex items-center justify-between">
                      ...
                      <span className="font-bold text-indigo-600">
                        Rp {formatNumber(sumByTrxType("cash"))}
                      </span>
                    </div>
