...
const RevenueReport = ({ warehouseBalance }) => {
  ...
  const { revenue, error, isLoading, isValidating } = useRevenueReport(
  ...
  const fetchCorpCashFlows = useCallback(async () => {
    try {
      ...
      setCorpCashFlows(response.data?.data?.cash_flows);
      setCorpCashFlowsGrouped(response.data?.data?.cash_flows_grouped);
    } catch (err) {
      setNotification(
        err.response?.data?.message || "Failed to fetch corp expense data",
      );
  ...
  useEffect(() => {
    fetchCorpCashFlows();
  }, [fetchCorpCashFlows]); // Gunakan property spesifik sebagai dependency
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between p-4 rounded-xl bg-white border border-slate-100 dark:bg-slate-900 dark:border-slate-800">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="truncate">Cashflow Corp.</span>
          </button>
        </div>
      </div>
      ...
      <Modal
        ...
        title="Tambah Pengeluaran Owner/Corporate"
      >
        <CreateExpenseCorp
          setIsModalOpen={setIsModalOpen}
          notification={setNotification}
          fetchCorpCashFlows={fetchCorpCashFlows}
        />
      </Modal>
    </>
  );
