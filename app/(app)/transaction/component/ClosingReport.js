...
export default function ClosingReport({
  ...
  useEffect(() => {
    ...
    const lockWhId =
      localStorage.getItem(`lock_warehouse_id_${warehouseId}`) ||
      localStorage.getItem("lock_warehouse_id");
    const targetLockTime =
      localStorage.getItem(`target_lock_time_${warehouseId}`) ||
      localStorage.getItem("target_lock_time");
    const savedTelegram =
      localStorage.getItem(`last_telegram_data_${warehouseId}`) ||
      localStorage.getItem("last_telegram_data");
  ...
  const handleCloseStore = async () => {
    ...
    try {
      const { data: warehouseStatus } = await axios.get(
      ...
      const telegramResponseObj =
        result?.telegramData?.data || result?.telegramData;
      setRawTelegramData(telegramResponseObj);
      localStorage.setItem(
        `last_telegram_data_${warehouseId}`,
      ...
      if (notification)
        notification("Shift berhasil ditutup & Laporan Telegram dikirim!");
  ...
  const cabang = rawText
    ? extractValue(/📍 Sumber:\s*([^\n]+)/, rawText)
    : warehouseName;
  const tanggal = rawText
    ? extractValue(/📝 Detail:\s*\n([^\n]+)/, rawText)
    : today;
  ...
  const totalSetoranTelegram = extractValue(
    /Total Setoran:\s*([^\n]+)/,
    rawText,
  );
  ...
  const dispatchTime = tgResult?.date
    ? new Date(tgResult.date * 1000)
    : new Date();
  ...
  const qrValue =
    rawText ||
    copyDailyReportTelegram() ||
    `CLOSING_SHIFT|STORE:${warehouseName}|AMOUNT:${finalSetoran}|DATE:${today}|TG_MSG:${telegramMsgId || "OK"}`;
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-100 flex items-center gap-1.5">
                  Laporan Telegram Terverifikasi
                </h4>
