...
const TransactionContent = () => {
  ...
  const warehouseOpenCash =
    user?.warehouse?.primary_cash?.limit?.limit_amount || 9000000;
  ...
  const warehouseCashId = warehouses.find(
  ...
  const whAccounts = useMemo(
    () => accounts.filter((account) => account.warehouse_id === warehouseId),
    [accounts, warehouseId],
  );
  const hqAccounts = useMemo(
    () => accounts.filter((account) => account.warehouse_id === 1),
    [accounts],
  );
  const hqAccountIds = useMemo(
    () => hqAccounts.map((account) => account.id),
    [hqAccounts],
  );
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return journalByWarehouse.filter((journal) => {
      const matchAccount =
        accountFilter !== "all" &&
        (Number(journal.cred_id) === Number(accountFilter) ||
          Number(journal.debt_id) === Number(accountFilter));

      const matchCategory =
        categoryFilter !== "all" && journal.trx_type === categoryFilter;
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        return storedSetting
          ...
          : { feeAdminAuto: false, altFee: false };
      } catch (e) {
