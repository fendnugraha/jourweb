...
const FlowStat = ({ icon: Icon, label, value, color }) => (
  <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-slate-800/40">
    <span
      className={`text-[10px] flex items-center gap-1 font-sans text-slate-400`}
    >
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <h4 className="text-xs font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider truncate">
                {title}
              </h4>
              <span
                className={`inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-extrabold font-mono shrink-0 ${c.badge}`}
              >
                {percentage}%
              </span>
            </div>
...
const CashBankSummary = ({
  ...
  const mutationInSumById = useCallback(
    (acc_id) =>
      journals.reduce(
        (sum, j) =>
          Number(j.debt_id) === Number(acc_id) && j.trx_type === "Mutasi Kas"
            ? sum + Number(j.amount)
            : sum,
        0,
      ),
  ...
  const mutationOutSumById = useCallback(
    (acc_id) =>
      journals.reduce(
        (sum, j) =>
          Number(j.cred_id) === Number(acc_id) && j.trx_type === "Mutasi Kas"
            ? sum + Number(j.amount)
            : sum,
        0,
      ),
  ...
  const totals = useMemo(() => {
    ...
    return {
      kas: {
        ...
        percentage:
          grandTotalBalance > 0
            ? Math.max(0, (kasBalance / grandTotalBalance) * 100).toFixed(1)
            : "0.0",
      },
      bank: {
        balance: bankBalance,
        ...
        percentage:
          grandTotalBalance > 0
            ? Math.max(0, (bankBalance / grandTotalBalance) * 100).toFixed(1)
            : "0.0",
      },
      grandTotalBalance,
    };
        ...
        <div className="p-3.5 sm:p-4 rounded-2xl bg-slate-700 dark:bg-slate-900 text-white border border-slate-800 dark:border-indigo-900/50 space-y-3 shadow-sm">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <h5 className="text-[11px] font-medium text-slate-300 truncate">
                  Grand Total Saldo
                </h5>
            ...
            <span className="text-sm sm:text-base font-extrabold font-mono text-amber-400 shrink-0">
