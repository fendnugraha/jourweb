        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 p-5 rounded-2xl shadow-sm space-y-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <div className="flex items-center gap-1.5">
                            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">KAS / BANK</h3>
                            {/* Indikator Revalidating (Sync di background) */}
                            {isValidating && !isLoading && <RefreshCw className="w-3 h-3 text-indigo-500 animate-spin" title="Memperbarui data..." />}
                        </div>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Ringkasan Saldo Akun</p>
                    </div>
                </div>
            ...
            <div className="relative">
                <AnimatePresence mode="wait">
                    ...
                    {isLoading ? (
                        <motion.div key="loading-skeleton" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-1">
                            {Array.from({ length: 4 }).map((_, index) => (
                                <div key={index} className="flex items-center justify-between py-2.5 px-3 -mx-1 animate-pulse">
                        ...
                        <motion.div key="account-list" variants={containerVariants} initial="hidden" animate="show" exit="hidden" className="space-y-1">
                            {accounts.map((account) => (
                                ...
                                >
                                    ...
                                    <div className="min-w-0 flex-1 pr-4">
                                        ...
                                        <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium truncate mt-0.5">{account.name}</p>
                                    </div>

                                    {/* Bagian Kanan: Saldo */}
                                    <div className="text-right shrink-0 flex flex-col items-center gap-1.5">
                                        <span
