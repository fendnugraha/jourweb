...
import { ArrowRightLeft, Calendar, Coins, CreditCard, FileWarning, Trash2 } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { useMemo, useState } from "react";

const MutationHistoryLog = ({ journals, warehouseId, setNotification, mutate, mutateBalance, searchTerm, accountFilter, accountOptions, userRole }) => {
    ...
    if (filteredTransactions.length === 0) {
        return (
            ...
            >
                ...
                <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">No mutation records found</p>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Try adjusting your search or filter settings.</p>
            </motion.div>
        );
    }

    return (
        <div className="space-y-4 max-h-131.25 overflow-y-auto">
            <AnimatePresence initial={false}>
                <div className="grid gap-2.5">
                    {filteredTransactions.map((tx, index) => {
                        const isOutflow = [...accountOptions.map((o) => o.value)].includes(Number(tx.cred_id));
                            ...
                            >
                                ...
                                <div className="flex items-start gap-3 min-w-0 flex-1">
                                    ...
                                    <div className="min-w-0 flex-1 space-y-1">
                                        ...
                                        <p className="text-xs font-bold leading-snug text-slate-800 dark:text-slate-100 wrap-break-word">{tx.description}</p>
                                ...
                                <div className="flex shrink-0 flex-col items-end justify-between self-stretch pl-2">
                                    <div className="text-right">
                                        <span className={`block font-mono font-bold ${isOutflow ? "text-red-500" : "text-green-500"}`}>
