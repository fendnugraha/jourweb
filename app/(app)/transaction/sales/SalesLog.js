...
const SalesLog = ({ txByWarehouse, mutate, mutateJournal, notification }) => {
    ...
    return (
      txByWarehouse.list?.filter((tx) => {
        ...
        const matchesCategory =
          categoryFilter === "all" || tx.product?.category === categoryFilter;
  ...
  const totalRevenue = useMemo(
    () =>
      filteredTransactions.reduce(
        (s, tx) => s + tx.price * Math.abs(tx.quantity),
        0,
      ),
  ...
  const totalCost = useMemo(
    () =>
      filteredTransactions.reduce(
        (s, tx) => s + tx.cost * Math.abs(tx.quantity),
        0,
      ),
    [filteredTransactions],
  );
  ...
  const totalQty = useMemo(
    () => filteredTransactions.reduce((s, tx) => s + Math.abs(tx.quantity), 0),
    [filteredTransactions],
  );
    ...
    <div className="space-y-4">
      ...
      >
        {[
          ...
          >
            <div
              className={`p-2 rounded-xl bg-white/60 dark:bg-slate-900/40 ${stat.color}`}
            >
            ...
            <div className="min-w-0">
              ...
              <p
                className={`text-xs font-bold font-mono truncate ${stat.color}`}
              >
                {stat.value}
              </p>
            </div>
