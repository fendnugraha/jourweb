...
export default function PointOfSale({
  ...
  const posProducts = useMemo(() => {
    return stockItems.map((item) => {
      const retailPrice =
        item.price > 0 ? item.price : Math.round((item.cost * 1.8) / 100) * 100;
      return {
        id: item.id,
        name: item.name,
        price: retailPrice,
  ...
  const filteredProducts = useMemo(() => {
    return posProducts.filter((product) => {
      ...
      const matchesSearch =
        ...
        (skuVal && skuVal.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleOpenPreview = (e) => {
    ...
    setIsMobileCartOpen(false); // Tutup drawer mobile jika terbuka
  ...
  // Komponen Reusable untuk Form & List Keranjang
  const CartFormContent = () => (
    <div className="flex flex-col justify-between h-full space-y-3">
      ...
      {/* Cart items list */}
      <div className="flex-1 overflow-y-auto max-h-60 lg:max-h-55 my-2 space-y-2.5 pr-1">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="rounded-lg border border-slate-200 p-1 bg-white hover:bg-slate-100 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"
                >
                  <Minus className="h-3 w-3" />
                </button>
                <span className="w-6 text-center font-mono text-xs font-bold text-slate-800 dark:text-slate-100">
                  {item.quantity}
                </span>
                <button
                  type="button"
        ...
        {cart.length === 0 && (
          <div className="h-36 flex flex-col items-center justify-center text-center p-4">
            ...
            <p className="text-[10px] text-slate-400/80 mt-0.5 max-w-50">
              Pilih produk di katalog untuk ditambahkan ke keranjang
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="relative pb-36 lg:pb-0" id="pos-terminal-system">
      <div className="grid gap-6 lg:grid-cols-12">
        {/* LEFT SECTION: Catalog (Full width di Mobile, 7 Col di Desktop) */}
        <div className="lg:col-span-7 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-0.5 px-4 -mx-4 sm:mx-0 sm:px-0">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold rounded-xl whitespace-nowrap transition-all shrink-0 ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-h-[calc(100vh-280px)] lg:max-h-145 overflow-y-auto pr-1">
            {filteredProducts.map((product) => {
              ...
              const currentStock =
                "currentStock" in product ? product.currentStock : Infinity;
              ...
              return (
                <motion.button
                  ...
                  type="button"
                  whileTap={{
                    scale: isOutOfStock || isLimitReached ? 1 : 0.96,
                  }}
        ...
        {/* RIGHT SECTION: Cart Desktop (Hanya muncul di Layar Besar lg:) */}
      ...
      {/* ========================================================================= */}
      {/* MOBILE FLOATING BOTTOM BAR (Hanya Tampil di Layar Mobile/Tablet < lg) */}
      {/* ========================================================================= */}
      <div className="lg:hidden fixed bottom-16 sm:bottom-20 left-3 right-3 z-40">
        <motion.button
          ...
          className="w-full bg-slate-900 dark:bg-indigo-600 text-white rounded-2xl p-3.5 shadow-xl flex items-center justify-between border border-slate-800 dark:border-indigo-500"
        >
        ...
        {isMobileCartOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex flex-col justify-end">
            ...
            <motion.div
              ...
              className="relative w-full bg-white dark:bg-slate-900 rounded-t-3xl p-5 shadow-2xl border-t border-slate-200 dark:border-slate-800 max-h-[85vh] flex flex-col"
            >
              {/* Drag Handle Top */}
      ...
      {/* ========================================================================= */}
      {/* MODAL PREVIEW & RECEIPT (TETAP SAMA SEPERTI VERSI AWAL) */}
      {/* ========================================================================= */}
      <AnimatePresence>
        {isPreviewOpen && (
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400">
                      Periksa item sebelum menyelesaikan transaksi
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="grid grid-cols-2 gap-3 mt-5">
                <button
                  type="button"
                  onClick={() => setIsPreviewOpen(false)}
                  className="py-2 px-3 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-600 dark:text-slate-300"
                >
                  Kembali & Ubah
                </button>
                <button
                  ...
                  className="inline-flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-indigo-600 text-white text-xs font-bold shadow-md"
                >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="text-center pb-4 border-b border-dashed border-slate-200 dark:border-slate-800">
                ...
                <h2 className="text-base font-bold text-slate-900 dark:text-slate-50 uppercase tracking-wide">
                  BRILink THREEKOMUNIKA
                </h2>
                ...
                <table className="w-full text-[11px] font-mono">
                  ...
                  <tbody className="divide-y divide-slate-100/50 dark:divide-slate-800/30">
                    {lastReceipt.items.map((item) => (
                      ...
                      >
                        <td className="py-1.5 pr-2 truncate max-w-32">
                          {item.name}
                        </td>
              ...
              <div className="grid grid-cols-2 gap-3 mt-5">
                <button
                  type="button"
                  onClick={handlePrint}
                  className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 dark:border-slate-800 dark:bg-slate-800 dark:text-slate-300"
                >
                ...
                <button
                  type="button"
                  onClick={handleNewOrder}
                  className="inline-flex items-center justify-center gap-1 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-semibold text-white shadow-xs"
                >
                  Order Baru
