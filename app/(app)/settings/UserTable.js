            ...
            <div className="overflow-x-auto">
                <table className="w-full border-collapse text-left">
                    ...
                    <tbody className="divide-y divide-slate-100 text-xs dark:divide-slate-800">
                        {users?.map((user) => (
                            <tr key={user.id} className="group hover:bg-slate-50/40 dark:hover:bg-slate-850/20 transition-colors duration-150">
                                <td className="px-6 py-4">
                                    {user.name}
                                    <span className="block text-xs text-slate-500 dark:text-slate-400">{user.contact?.name || "N/A"}</span>
                                </td>
                                <td className="px-6 py-4">{user.email}</td>
                                <td className="px-6 py-4">{user.role}</td>
                                <td className="px-6 py-4">{user.warehouse?.name}</td>
