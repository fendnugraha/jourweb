...
import { DateTimeNow, formatDateTime, formatRupiah, todayDate } from "@/app/utils/format";
...
export default function MyProfile() {
    ...
    const attendanceMap = (contactMonthly?.attendances || []).reduce((acc, curr) => {
        if (curr?.date) {
            const cleanDate = curr.date.slice(0, 10); // Ambil format YYYY-MM-DD
            acc[cleanDate] = curr;
        }
        return acc;
    }, {});
            ...
            <div className="max-w-6xl mx-auto space-y-6 pb-12 text-slate-800 dark:text-slate-100">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-1">
                                <div className="flex items-center gap-2 flex-wrap">
                                    <h1 className="text-xl sm:text-2xl font-bold capitalize tracking-tight text-slate-900 dark:text-white">{user.name}</h1>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h2 className="text-base font-bold text-slate-900 dark:text-white">Kalender Absensi</h2>
                                <p className="text-xs text-slate-400">Klik pada tanggal yang memiliki indikator untuk melihat detail kehadiran</p>
                            </div>
                        </div>
                        ...
                        {Array.from({ length: daysInMonth }).map((_, idx) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {attendance && (
                                        <>
                                            <span className="hidden sm:inline font-semibold text-xs text-right">{attendance.time_in.slice(0, 5)}</span>
                                            <div className="space-y-1">
                                                {(() => {
                                                    ...
                                                    return (
                                                        <span
                                                            className={`x-1.5 py-0.5 rounded text-[10px] font-bold flex items-center justify-between ${color}`}
                                                        >
                ...
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Detail Kehadiran">
                    <div className="flex items-center gap-2.5 border-b border-slate-100 dark:border-slate-800 pb-3">
                        ...
                        <h3 className="text-base font-bold text-slate-900 dark:text-white">Detail Kehadiran ({formatDateTime(selectedAttendance?.date)})</h3>
                    </div>
                        ...
                        {user.warehouse && (
                            <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                                    <div>
                                        <span className="text-slate-400 dark:text-slate-500 block text-[11px] mb-1">Nama Outlet / Konter</span>
                                        <p className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{user.warehouse.name}</p>
                                    </div>

                                    <div>
                                        <span className="text-slate-400 dark:text-slate-500 block text-[11px] mb-1">Jam Operasional & Status</span>
                                    ...
                                    <div className="sm:col-span-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
                                        <span className="text-slate-400 dark:text-slate-500 block text-[11px] mb-1">Alamat Lengkap</span>
