...
import { Package, Tag, DollarSign, Coins, ShieldCheck, Edit2, Search, Box, Plus } from "lucide-react";
...
const ProductTable = () => {
    ...
    const categoryOptions = [{ value: "all", label: "All Categories" }, ...categoryProducts.map((cat) => ({ value: cat.name, label: cat.name }))];
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between p-4 rounded-xl bg-white border border-slate-100 dark:bg-slate-900 dark:border-slate-800">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <h1 className="text-xl font-bold">
                        {filteredProducts?.length || 0} <span className="text-slate-500 font-semibold">Products</span>
                    </h1>
                    <button
                        type="button"
                        onClick={() => {
                        ...
                        <tbody className="divide-y divide-slate-100 text-xs dark:divide-slate-800/60">
                            {filteredProducts?.length > 0 ? (
                                filteredProducts.map((product) => (
                                    <tr key={product.id} className="group hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors duration-150">
                                        ...
                                        {/* 3. Cost (Harga Modal) */}
                                        <td className="px-5 py-4 text-right font-mono text-slate-500 dark:text-slate-400">{formatNumber(product.cost)}</td>
                                            ...
                                            >
                                                <span
                                                    className={`h-1.5 w-1.5 rounded-full ${
                                                        product.is_active || product.status === "active" ? "bg-emerald-500 animate-pulse" : "bg-rose-500"
                                                    }`}
                                                />
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400 dark:text-slate-500">
                                        <div className="flex flex-col items-center justify-center gap-2">
                                            ...
                                            <p className="text-xs font-medium">Tidak ada data produk ditemukan.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalTitle} maxWidth="max-w-md">
                {modalName === "create" && (
                    <CreateProduct isModalOpen={setIsModalOpen} categoryProducts={categoryProducts} notification={setNotification} mutate={mutate} />
                )}
                {modalName === "edit" && (
                    <EditProduct
