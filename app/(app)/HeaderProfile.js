...
export default function HeaderProfile() {
  ...
  const contactWarningStatus =
    user?.attendances?.[0]?.contact?.employee?.warning_active || false;
  ...
  const warehouseRankIndex = rankByProfit?.revenue?.findIndex(
    (item) => Number(item.warehouse_id) === Number(userWarehouseId),
  );
  const WarehouseRank =
    warehouseRankIndex !== -1 && warehouseRankIndex !== undefined
      ? warehouseRankIndex + 1
      : 0;
  const WarehouseRankProfit =
    rankByProfit?.revenue?.find(
      (item) => Number(item.warehouse_id) === Number(userWarehouseId),
    )?.total || 0;
  const WarehouseMonthlyProfit =
    rankByProfit?.totalProfitMonthly?.find(
      (item) => Number(item.warehouse_id) === Number(userWarehouseId),
    )?.average_profit || 0;
      ...
      <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-start sm:items-center gap-3.5 sm:gap-4">
          ...
          <div className="space-y-1 min-w-0 flex-1">
            ...
            <h2 className="text-lg sm:text-xl font-extrabold tracking-tight text-slate-900 dark:text-slate-50 truncate">
              Welcome Back,{" "}
        ...
        <div className="flex items-center gap-2.5 overflow-x-auto pb-1 lg:pb-0 no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-2.5 px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 shrink-0">
                ...
                <div>
                  <div className="text-[9px] sm:text-[10px] font-bold tracking-wider text-slate-400 capitalize">
                    Rank
                  </div>
