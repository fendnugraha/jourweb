...
function CollapsibleSection({ title, subtitle, total, items, icon: Icon, iconColor, delay = 0, isLoading = false, isOpen, onToggle }) {
    return (
        <motion.div
...
export default function FinancialReport() {
    ...
    const corporateExpenses = useMemo(() => {
        ...
        return corpExpense
            .filter((item) => item.is_corporate === 1 && item.type === "expense")
            .map((item) => ({
    ...
    const branchExpenses = useMemo(() => {
        const items = Array.isArray(corpExpense)
            ? corpExpense
                  .filter((item) => item.is_corporate === 0 && item.type === "expense")
                  .map((item) => ({
                      label: item.category || item.name || "Lainnya",
                      amount: Number(item.total || 0),
                      icon: Receipt,
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            // Buka semua collapsible section agar rendernya lengkap
            setOpenBranch(true);
            setOpenCorporate(true);

            // Beri sedikit jeda animasi DOM
            ...
            const dataUrl = await toPng(element, {
                quality: 0.95,
                cacheBust: true,
                pixelRatio: 2, // Meningkatkan resolusi PDF agar teks tajam
            });
            ...
            // Handle halaman berikutnya jika konten sangat panjang
            while (heightLeft > 0) {
            ...
            pdf.save(`Laporan_Keuangan_${formattedPeriodName.replace(/\s+/g, "_")}.pdf`);
        } catch (error) {
            console.error("Gagal mengeksport PDF:", error);
        } finally {
            ...
            >
                ...
                <div className="flex items-center gap-2 self-start sm:self-auto">
                    ...
                    <button
                        ...
                        disabled={isExporting || isMainLoading}
                        className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs transition-all disabled:opacity-50 cursor-pointer"
                    >
                        {isExporting ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}
                        <span>{isExporting ? "Mengunduh..." : "Export PDF"}</span>
                    </button>
                </div>
            </motion.div>

            {/* Wrapper Ref Konten Laporan (Elemen yang ditangkap ke PDF) */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="space-y-4">
                        ...
                        <CollapsibleSection
                            ...
                            isLoading={isExpenseLoading}
                            delay={0.38}
                            isOpen={openCorporate}
                            onToggle={() => setOpenCorporate((v) => !v)}
                        />
                    </div>
                    ...
                    >
                        ...
                        <div className="px-5 py-3">
                            ...
                            <PLRow
                                label="Pengeluaran Corporate/Owner"
                            ...
                            <div className="mt-4 space-y-2.5 pb-2">
                                {[
                                    {
                                        label: "Gross Margin",
                                        value: Math.max(0, Math.min(100, parseFloat(grossMargin))),
                                        color: "bg-amber-400",
                                    },
                                    {
                                        label: "Net Margin",
                                        value: Math.max(0, Math.min(100, parseFloat(netMargin))),
