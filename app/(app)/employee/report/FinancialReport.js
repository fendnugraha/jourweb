      ...
      <div className="w-full pt-2">
        ...
        <div className="flex justify-between items-center mt-2 px-1">
          {MONTHS.map((m) => (
            <span
              key={m}
              className="text-[9px] font-medium text-slate-300 dark:text-slate-700"
            >
              {m}
            </span>
          ))}
...
export default function FinancialReport() {
  ...
  const { startPeriod, endPeriod, formattedPeriodName } = useMemo(() => {
    if (!period)
      return { startPeriod: null, endPeriod: null, formattedPeriodName: "" };
  ...
  const { revenue, isLoading: isRevenueLoading } = useRevenueReport(
    startPeriod,
    endPeriod,
  );
  ...
  const totalRevenue = useMemo(() => {
    ...
    return revenue.revenue.reduce(
      (sum, item) => sum + (Number(item.fee) + Number(item.expense) || 0),
      0,
    );
  }, [revenue]);
  ...
  const totalRevenueLastMonth = useMemo(() => {
    if (!revenue || !Array.isArray(revenue.revenue)) return 0;
    return revenue.revenue_last_month.reduce(
      (sum, item) => sum + (Number(item.fee) + Number(item.expense) || 0),
      0,
    );
  }, [revenue]);
    ...
    const fetchMonthlyPayrollSum = async () => {
      try {
        const response = await axios.get(
          `/api/monthly-payroll-sum/${endPeriod.split("T")[0]}`,
        );
  ...
  const totalBranch = useMemo(
    () => branchExpenses.reduce((s, e) => s + e.amount, 0),
    [branchExpenses],
  );
  const totalCorporate = useMemo(
  ...
  useEffect(() => {
    const fetchMonthlyRevenue = async () => {
      ...
      try {
        const response = await axios.get(
          `/api/yearly-profit-report/${yearStr}`,
        );
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <div>
            <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
              Tren Pendapatan (Gross Profit)
            </p>
            <p className="text-[10px] text-slate-400 dark:text-slate-500">
              Grafik Pergerakan Bulanan
            </p>
          </div>
          {isRevenueLoading ? (
