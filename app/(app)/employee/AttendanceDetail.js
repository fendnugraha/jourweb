  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case "Late":
        return {
          label: "Late",
          class: "bg-rose-50 text-rose-500 border-rose-200",
        };
      case "Overtime":
        return {
          label: "Lembur",
    ...
    <div className="w-full text-slate-700 font-sans antialiased space-y-2">
      ...
      <div className="flex items-center justify-between pb-3">
        <div>
          ...
          <h3 className="text-base font-extrabold text-slate-800 tracking-tight">
            {selectedWarehouse?.name || "Nama Outlet"}
          </h3>
        </div>

        {attendance?.approval_status && (
          <span
            className={`px-3 py-1 rounded-full text-xs font-semibold border ${statusInfo.class}`}
          >
            {statusInfo.label}
          </span>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-stretch">
        ...
        <div className="md:col-span-5 flex flex-col">
          ...
          <div className="relative w-full flex-1 min-h-55 rounded-2xl overflow-hidden bg-slate-50 border border-slate-200/80 flex items-center justify-center group shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                {attendance?.created_at ? (
                  ...
                  <div className="px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-400 italic">
                    Belum Absen
                  </div>
                )}
              </div>
