...
const EditEmployee = ({
  ...
  const handleDeleteComponent = async (componentId) => {
    try {
      ...
      const errorMsg =
        error?.response?.data?.message ||
        "An error occurred while deleting the component.";
      notification(errorMsg);
    }
  };
        ...
        <>
          <form onSubmit={handleSubmitComponent}>
            <div className="flex gap-2">
              ...
              <Dropdown
                ...
                onChange={(val) =>
                  setFormDataComponent({ ...formDataComponent, type: val })
                }
              />
              <button
                type="submit"
          ...
          <div className="mt-2 border rounded-xl border-slate-300 dark:border-slate-700">
            <table className="min-w-full divide-y divide-slate-300 dark:divide-slate-700">
              <thead>
                <tr>
                  <th className="p-4 text-xs font-medium text-slate-500 dark:text-slate-400 text-left">
                    Component Name
                  </th>
                  <th className="p-4 text-xs font-medium text-slate-500 dark:text-slate-400 text-center">
                    Amount
                  </th>
                  <th className="p-4 text-xs font-medium text-slate-500 dark:text-slate-400 text-center">
                    Action
              ...
              <tbody>
                {employee.salary_components?.map((component) => (
                  ...
                  >
                    ...
                    <td
                      className={`px-4 py-2 text-sm ${component.type === "deduction" ? "text-red-500" : "text-green-500"} text-right`}
                    >
