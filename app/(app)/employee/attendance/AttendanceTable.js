...
const STATUS_CONFIG = {
    ...
    Pending: {
        ...
        Icon: Clock,
        color: "bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-200/80 dark:border-amber-800/50",
        iconStyle: "w-3.5 h-3.5 text-amber-500 dark:text-amber-400",
    },
};
...
const renderDiffBadge = (targetOpeningTime, timeIn, status) => {
    ...
    const isLate = status === "Late" || (actual > start && status !== "Approved" && status !== "Good");

    if (isLate) {
        const diff = diffTimeHuman(start, actual);
        if (!diff) return null;
        return (
            <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/50 px-2 py-0.5 rounded-md border border-rose-200/60 dark:border-rose-900/50 whitespace-nowrap">
    ...
    if (!diff) return null;
    return (
        <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 px-2 py-0.5 rounded-md border border-emerald-200/60 dark:border-emerald-900/50 whitespace-nowrap">
