...
export default function DeliveryContent() {
  ...
  const deliveriesData = deliveries?.map((item) => ({
    ...
    source_warehouse: {
      ...
      lat: item.source_account?.warehouse?.latitude
        ? Number(item.source_account.warehouse.latitude)
        : null,
      lng: item.source_account?.warehouse?.longitude
        ? Number(item.source_account.warehouse.longitude)
        : null,
    },
    ...
    destination_warehouse: {
      ...
      lat: item.destination_account?.warehouse?.latitude
        ? Number(item.destination_account.warehouse.latitude)
        : null,
      lng: item.destination_account?.warehouse?.longitude
        ? Number(item.destination_account.warehouse.longitude)
        : null,
    },
  ...
  const stats = useMemo(() => {
    ...
    const inTransitCount = deliveriesData.filter(
  ...
  const statusCounts = useMemo(() => {
    return {
      ...
      in_transit: deliveriesData.filter((i) => i.status === "in_transit")
        .length,
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white p-4 sm:p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900 space-y-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">
                Pusat kontrol & distribusi dana kas ke cabang/bank
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <RotateCw
              className={`w-4 h-4 ${isValidating ? "animate-spin text-indigo-500" : ""}`}
            />
          </button>
        </div>
              ...
              {filteredData.length > 0 ? (
                filteredData.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      className="hover:bg-slate-50/40 dark:hover:bg-slate-850/20"
                    >
                ...
                <tr>
                  <td
                    colSpan={6}
                    className="px-6 py-10 text-center text-slate-400 font-sans"
                  >
