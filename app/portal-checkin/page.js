...
const PortalCheckin = () => {
  ...
  useEffect(() => {
    if (user?.has_checked_in && !attendance) {
      router.push("/transaction");
    }
  }, [user?.has_checked_in, attendance, router]);
      ...
      if (response.data.found && response.data.warehouse) {
        ...
        setError(
          response.data.message ||
            "Kamu sedang di luar jangkauan lokasi absen.",
        );
        setWarehouseId(null);
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      ...
      try {
        const response = await axios.post("/api/create-attendance", formData, {
  ...
  if (attendance) {
    return (
      <CheckInSuccess
        ...
        onContinue={() => router.push("/transaction")}
      />
    );
  }
              ...
              {preview ? (
                ...
                <div className="flex flex-col items-center py-2">
                  ...
                  <p className="text-[11px] text-slate-500 max-w-xs mb-3">
                    {location
                      ? "Kamera siap, pastikan wajah terlihat jelas ya."
                      : "Nyalakan GPS dulu untuk bisa ambil foto."}
                  </p>
              ...
              <div>
                ...
                <span className="font-mono font-semibold text-slate-200">
                  {timeIn || (
                    <LiveClock
                      textSize="text-xs"
                      style="font-mono font-semibold text-slate-200"
                    />
                  )}
                </span>
              </div>
            ...
            <div className="p-3 bg-slate-800/30 rounded-xl border border-slate-800/50 flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                ...
                <span className="text-[11px] text-slate-300 truncate block">
                  {address
                    ? address.city || address.suburb || "Terdeteksi"
                    : "Menunggu GPS..."}
                </span>
              </div>
            </div>
